"""
JESSE:
This function was copied from the code of the paper, it shows how they create the feature map of the vgg19 loss function. 
However there is an other discrapency between code and paper, the paper states that they take the output from the cnn right before the 5e 
max pooling layer. here they just take the output from the max pooling lager. 
"""
...
#        assert bgr.get_shape().as_list()[1:] == [224, 224, 3]
        ...
        network = Conv2d(network, n_filter=512, filter_size=(3, 3), strides=(1, 1), act=tf.nn.relu, padding='SAME', name='conv5_4') #Jesse: according to the paper take this output. 
        network = MaxPool2d(network, filter_size=(2, 2), strides=(2, 2), padding='SAME', name='pool5')  # (batch_size, 7, 7, 512) #Jesse: according to the code this output is taken. 
        conv = network
        """ fc 6~8 """
...
#        network = DenseLayer(network, n_units=1000, act=tf.identity, name='fc8')
        return network, conv #Jesse: outputs are the same right?